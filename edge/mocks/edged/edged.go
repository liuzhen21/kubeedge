...
func Register(e *v1alpha2.Edged) {
	...
	edged, err := newEdged(e.Enable, e.HostnameOverride, e.RegisterNodeNamespace, e.Count)
	if err != nil {
...
func (e *edged) Start() {
	...
	go func() {
		for i := 0; i < e.count; i++ {
			go func(i int) {
				e.KubeletServer.HostnameOverride = fmt.Sprintf("%s-%d", e.nodeName, i)
...
func newEdged(enable bool, nodeName, namespace string, count int) (*edged, error) {
	var ed *edged
	var err error
	if !enable {
		return &edged{
...
func (e *edged) syncPod(podCfg *config.PodConfig) {
	...
	for {
		...
		klog.Infof("receive msg %s %s %s", resType, op, content)
		switch resType {
